...
struct key_val_pair
{
  ...
  key_val_pair(std::string n, int code) : name(std::move(n)), bin_code(code)
  {
  }
};
...
class map
{
 ...
 public:
  ...
  bool find(const std::string& op)
  {
    ...
    // try using std::any_of() for the below checking later when done with the
    // project
...
int main()
{
   ...
   * output file -> (intermediate file) going to be populated with the 1st pass
   * of the Two-Pass assembler
   *
   */
  std::ifstream input_file("my.txt");
  std::fstream intermediate_file(
      "outda.txt", std::ios::out);  // change to "intermediate.txt"
  ...
  /*
   * op_tab -> static DS consisting of instructions in the SIC, along with their
   * opcode for now. I plan to include the instruction size in this field too,
   * to make this upward compatible for SIC/XE.
   *
   * sym_tab -> dynamic DS that will hold the pair (LABEL, ADDR_OF_LABEL).
   * Future plans include to add in a new field to account for DEFINED,
   * MULTI_DEFINED, UNDEFINED to help for better debugging while producing the
   *            listing file.
   *
   * LOC_CTR -> keeps track of the next instruction's location, WHERE the
   * instruction encountered HAS to be written.
   *
   * START_ADDR -> stores where to begin assigning the instructions from, the
   * START address of the program that can help the loader.
   */
  map op_tab{};
  ...
  /*
   * line -> a std::istringstream object, through which I will be extracting the
   * LABEL, OPCODE and OPERAND fields appropriately.
   *
   * label, opcode and operand are going to store the individuals fields
   * extracted through the istringstream object.
   *
   * add_to_loc -> WHERE should the next instruction begin from, stores that
  ...
  /* check below later, as I needed to do this ONLY because I added 1 to the
   * "END" label occurrence. I think we can safely neglect that?
   *
  ...
  /*
   * close the intermediate file to which we were writing to, and then re-open
   * it in read mode.
   *
  ...
  if (!intermediate_file.is_open())
  {
    std::cerr << "Could not open the Intermediate File to read from in the 2nd "
                 "pass\n";
    exit(0);
  }
  ...
  /*
   * as we need 6 bytes for writing the PROG_NAME in the header record, I check
   * below and append x's if necessary
   *
   * (further, need to handle the case where the prog name is just way bigger
   * than 6 before itself)
   */
  while (prog_name.length() < 6)
    prog_name += 'x';

  /*
   * initialise an ostringstream object, which will combine strings together to
   * be further appended as a header record
   *
   *
   */

  /* updating to get the hex value, instead of the decimal equivalent, for the
   * START_ADDR and LENGTH_OF_PROG
   *
  ...
  std::cout << "H_" + prog_name + '_' << start_add_as_str << '_'
            << length_as_str << std::endl;
  obj_file << "H_" + prog_name + '_' << start_add_as_str << '_' << length_as_str
           << std::endl;

  /* cooking the text records (currently this is only done for the WORD and BYTE
   * opcodes)
   *
   * UPDATE:no, it doesnt work for JUST the above, i'm expanding to fit in the
   * whole thing itself now.
   */
  ...
  while (std::getline(intermediate_file, line))
  {
    ...
    if (iss >> loc >> label >> opcode >> operand)
    {
      ...
      {
        // get the length of the instruction (have not added that field in the
        // op_tab, but will have to do it for SIC/XE
        ...
        // convert the LABEL_ADDR that we got to a string, first converting to
        // hex
        ...
        /*
         * handle the text record's length by making sure that THE CURRENT
         * instruction can fit in the CURRENT text record. If it can, go ahead
         * and fill it in the CURRENT record. Else, write the current record to
         * the file, then re-initialize a new text record.
         */
      ...
      else if (opcode == "BYTE")
      {
        ...
        else if (operand[0] == 'X')
        {
          ...
          // n -> NUMBER of chars to be collected, NOT the last index to be
          // retreived
      ...
      else if (opcode == "WORD")
      {
         ...
         *
         * So, I first take in that integer (present as a string in "operand"
         * object), and then use the std::hex manipulator on the ostringstream
         * object to convert it to a hex string.
         */
        ...
        // do we need to clean this? IDTS because scope dies, and new object is
        // formed in the next iteration.
        ou.str("");
        ou.clear();
      ...
      {
        /*
         * This is an interesting bit. For records until now that have been
         * pushed into the 'put_next' object, we will have to flush it to the
         * object file because we don't want to write anything in this memory;
         * as it is `reserved` for variables.
         *
         * As of my understanding now, this is done because we want to
         * deliberately skip such records because we want to NOT WRITE anything
         * to this space, so the loader just starts writing from where-ever it
         * was instructed, and as this space was not written to, BUT was
         * reserved, this space is reserved for variables!
         *
         ...
         * STR2 RESW    2
         *
         * Now, during the encounter with "STR", the code just needs to skip,
         * even though we encountered a "RESW", there are no records that need
         * to be written. So, if space_taken is 0, we just need to find the
         * first instruction, so we continue.
         *
         * If it is NOT zero, then we flush the previous input, and then
         * re-initialize and move on.
         */
        ...
        /*
         * We make the below as 'false' as usual, as in previous such scenarios
         * of having "full" the current text record, we would have flushed and
         * gone below to the `!found_first` block, initialised the current
         * instruction and move on.
         *
         * BUT, we don't want to initialise with this, as this is not an
         * instruction, but rather directives.
         *
         * So, we set the `found_first` flag to false, implying to allow the
         * next instruction to use it BY COMBINING it with the continue keyword.
         */
    ...
    {
      /*
       * The only such instruction that exists in SIC is RSUB. It does not take
       * in any operands, just opcode.
       *
       * In this case, we read the 'location' into "loc" variable, and the
       * 'RSUB' into the "label" variable because of how the code is structured.
       *
       * But, "END" can also enter this scope, and hence I check before if IT IS
       * "END", and just continue looping back if it is.
       */
    ...
    else if (operand.empty())
    {
      ...
      // No need to find again if the opcode is valid or not. I'm lazy and hey,
      // it works.
      ...
      /*
       * handle the text record's length by making sure that THE CURRENT
       * instruction can fit in the CURRENT text record. If it can, go ahead and
       * fill it in the CURRENT record. Else, write the current record to the
       * file, then re-initialize a new text record.
       */
  ...
  // adding the END record to the object file
  obj_file << "E_" << start_add_as_str << std::endl;
  std::cout << "E_" << start_add_as_str << std::endl;
